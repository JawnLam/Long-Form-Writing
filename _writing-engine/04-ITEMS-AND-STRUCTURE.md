...
# 04 — ITEMS AND STRUCTURE

> **The Item-type definitions, relationship vocabulary, and composition rules that hold across all genres. Per-genre emphasis lives in chapter 02; this chapter is the underlying schema.**
...
Plus the cartridge backbone files (`_manuscript-manifest.md`, `_state.md`, `_outline.md`, `_voice-samples.md`, `_argument.md`, `_craft-log.md`, `_spine.md`, `_continuity.md`, `_promises.md`) which are not Items but structural files that organize them. See "Cartridge backbone files" near the end of this chapter.

## Universal status enum (load-bearing)

Every prose-bearing Item (Beat, Scene, Section, Chapter, Act) uses **one** canonical lifecycle:

```
planned → drafting → drafted → revising → revised → final
```

Non-fiction and dissertation **Section** Items add one optional value, `fact-checked`, between `revised` and `final`:
...
Wiki-links in Item bodies (`[[Foo]]`) target the **filename without the `.md` extension**. There is no other link resolution mechanism. The validator enforces that every wiki-link resolves to an actual file.
...
| Section | `<chapter>-<order>-<short-slug>.md` (chapter-prefixed) | `03-01-Hoshi-Opening.md` | `Items/Sections/` |
| Chapter | `Chapter-<NN>-<short-title>.md` | `Chapter-03-Family-Business-Persistence.md` | `Items/Chapters/` |
| Act | `Act-<N>-<short-title>.md` | `Act-2-The-Reversal.md` | `Items/Acts/` |
| Character | `<First-Last>.md` or `<Slug>.md` | `Maya-Chen.md` | `Items/Characters/` |
| Character-Bible *(v1.3.1)* | `<First-Last>.md` (mirrors Character filename) | `Maya-Chen.md` | `Items/Character-Bibles/` |
| Reader | `<Reader-Slug>.md` (Title-Case-Hyphenated) | `Skeptic.md`, `Impatient-Generalist.md` | `Items/Readers/` |
| Motif | `<Motif-Name>.md` (Title-Case-Hyphenated) | `Cold-as-Inheritance.md`, `The-Empty-Chair.md` | `Items/Motifs/` |
| Theme *(v1.3.1)* | `<Theme-Name>.md` (Title-Case-Hyphenated) | `Honesty-Under-Cost.md` | `Items/Themes/` |
| Timeline *(v1.3.2)* | `<Layer>-<Slug>.md` (descriptive) | `Story-Time-Three-Weeks.md`, `Family-History-1968-2026.md`, `Maya-Life-1984-2026.md` | `Items/Timelines/` |
| Inspiration *(v1.3.2)* | `<Inspiration-Slug>.md` | `Sonoma-Frost-2019-NYT.md` | `Items/Inspirations/` |
| Thread | `<Thread-Name>.md` (Title-Case-Hyphenated) | `Distributed-Legitimacy.md` | `Items/Threads/` |
| Source | `<Lastname>-<Short-Title>-<Year>.md` | `Beard-SPQR-2015.md` | `Items/Sources/` |
| Setting | `<Setting-Name>.md` | `The-Conservatory.md` | `Items/Settings/` |
| Note | `<YYYY-MM-DD>-<short-slug>.md` | `2026-06-02-bell-curve-observation.md` | `Items/Notes/` |
...
- **Wiki-links** in Item bodies always use the filename form, never the Item_ID
...
**Location:** `Items/Beats/` (flat folder; chapter-prefixing avoids collisions).
...
5. `## Beats` — ordered list of beats (with wiki-links to beat Items)
...
5. `## Sources Used` — wiki-links to Source Items
6. `## Threads Engaged` — wiki-links to Thread Items
...
**Location:** `Items/Sections/`.

**Note:** `fact-checked` is a non-fiction / dissertation-only status value between `revised` and `final`. Fiction Section Items (rare; most fiction uses Scenes) skip it.
...
lfw_scenes_using: []   # auto-populated from Scene Items
Date_Added:
Date_Modified:
Needs_Processing: false
```
...
5. `## Scenes using this setting` — wiki-links to Scene Items

**Naming:** `<Setting-Name>.md`. E.g., `The-Conservatory.md`.

**Location:** `Items/Settings/`.
...
**Location:** `Items/Readers/`.

See chapter 10-READER for the READER-SIMULATION activity that uses Reader Items. Chapter 12 §6 extends READER-SIMULATION with fiction-specific protocol (dramatic-question, page-turn-impulse, emotional-flatline detection).

## Motif (fiction primary; optional for screenplay/play) *(v1.2)*

A recurring sub-surface element — theme, image system, recurring object, gesture, symbol, sound — that surfaces across scenes and builds (or fails to build) across the manuscript. Fiction's first-class Item for thematic through-lines. Structural equivalent to non-fiction's Thread.
...
3. `## Where it appears` — wiki-links to Scene Items with a one-line note on what the motif does in each
...
Recurring participant. First-class Item.
...
The extended companion Item for POV-bearing characters, antagonists, and major supporting characters in long novels. Most secondary characters never get one. The Character Item captures function and arc; the Bible captures the depth that the prose draws on.
...
**Location:** `Items/Character-Bibles/` (separate folder; folder makes the bible/character distinction immediate).
...
**Location:** `Items/Themes/`.

See chapter 14 §4 for the full discipline and the THEME-CHECK activity.

## Timeline (fiction primary) *(v1.3.2)*

Multi-layer timeline Item. Distinct from `_continuity.md`'s embedded story-time timeline; provides per-layer source-of-truth where `_continuity.md` becomes the cross-layer reconciliation.
...
3. `## Chronological events` — table or list of dated events with related-Item wikilinks
...
Research-as-compost. Distinct from Source (which carries non-fiction citation discipline): Inspiration Items track the books, articles, films, conversations, and observations that shape the prose without being cited.
...
3. `## Where it appears` — wiki-links to Section Items that engage it
4. `## Sources that support it` — wiki-links to Source Items
...
| `precedes` / `follows` | Item ↔ Item of same type | Sequence within parent |
...
| `parallels` | Item ↔ Item | Body, structural parallels |
| `relates-to` | Item ↔ Item | Body, free-form connection |

## Composition rules

- **Beats are discrete.** Do not nest beats inside beats.
...
- **Characters, Readers, and Threads are not composed into other Items.** They're referenced (appears-in, engaged-in, simulated-by) but they don't have an "order in parent."
...
| `_state.md` | Single source of truth for current state, today's focus, Item-status snapshot | All cartridges | Template + chapter 03 |
...
| `_storyboard.md` *(v1.3.2)* | Scene-card view — every Scene Item in compact form | Optional; recommended once cartridge has ≥10 drafted Scenes | Template + chapter 15 §3 |
...
For prose-bearing Items (Beat / Scene / Section / Chapter / Act), the canonical lifecycle is:

- **planned** → Item exists with frontmatter; outline (beats) may or may not exist yet; no prose
- **drafting** → outline is done; prose is being written but not yet complete
- **drafted** → first-pass prose is complete
- **revising** → a revision pass is currently touching this Item
...
Non-prose Items (Character, Thread, Source, Setting, Note) use type-specific status vocabularies — see the "Universal status enum" table earlier in this chapter.

## Item file conventions

- **Markdown only.** No HTML, no special syntax that breaks in non-Obsidian editors.
- **YAML frontmatter required.** Every Item has it.
- **Wiki-style links** (`[[Item-Name]]`) for cross-Item references.
