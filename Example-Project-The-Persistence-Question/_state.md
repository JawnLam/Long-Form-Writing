---
Item_Prototype: LFW_State
Item_ID: "the-persistence-question-state"
Title: "The Persistence Question — State"
Date_Added: 2026-06-02
Date_Modified: 2026-06-02
Needs_Processing: false
lfw_manuscript: "the-persistence-question"
lfw_lifecycle_stage: drafting
lfw_genre: non-fiction
lfw_sessions_completed: 14
lfw_total_writing_hours: 22.5
lfw_last_session_date: 2026-06-02
lfw_next_session_default_activity: DRAFT
lfw_current_revision_pass: null
lfw_current_revision_round: 0
lfw_word_count_total: 18750
lfw_word_count_target: 75000
---

# The Persistence Question — State

## Current Lifecycle Stage

- **Stage:** drafting
- **Stage entry date:** 2026-05-20 (session 008)
- **Sessions completed in this stage:** 7
- **Sessions completed total:** 14

## Today's Focus

**Chapter 3 (Family-Business Persistence: The Hoshi Ryokan Case) — Section 3.1 outlining**

The book moves from Chapter 2 (Antarctic Treaty institutional case) into the family-business case. Section 3.1 needs to introduce the Hoshi Ryokan (a Japanese inn operating in the same family since 718 AD) as a worked example, then introduce the structural-conditions framework as it applies to family firms.

Beats needed for Section 3.1:
- Opening narrative scene (the ryokan today)
- Statistical framing (how rare is multi-century family business?)
- Introduction of the four-conditions framework as it applies to families
- Setup for the case study to follow

## Word Count Progress

| Scope | Current | Target |
|-------|---------|--------|
| Total | 18,750 | 75,000 |
| Chapter 1 (Introduction) | 6,200 | 7,500 |
| Chapter 2 (Antarctic Treaty) | 10,800 | 9,000 |
| Chapter 3 (Family Business) | 1,750 | 9,000 |
| Chapter 4+ | 0 | ~48,000 |

(Chapter 2 is currently over target — needs structural revision pass to cut, scheduled after Chapter 3 first-draft.)

## Item Status Snapshot

### Chapters

| Chapter | Status | Word count | Last touched |
|---------|--------|------------|--------------|
| [[Chapter-01-Introduction]] | drafted | 6,200 | 2026-05-28 |
| [[Chapter-02-The-Antarctic-Treaty]] | drafting | 10,800 | 2026-05-31 |
| [[Chapter-03-Family-Business-Persistence]] | drafting | 1,750 | 2026-06-02 |
| [[Chapter-04-Religious-Orders]] | planned | 0 | — |
| [[Chapter-05-Constitutional-Republics]] | planned | 0 | — |
| [[Chapter-06-Counter-Examples-Failed-Persistence]] | planned | 0 | — |
| [[Chapter-07-Integration]] | planned | 0 | — |
| [[Chapter-08-Conclusion]] | planned | 0 | — |

### Sections (selected — most recent activity)

| Section | Status | Word count | Last touched |
|---------|--------|------------|--------------|
| [[01-01-The-Question]] (Ch 1) | drafted | 1,400 | 2026-05-26 |
| [[01-02-Four-Conditions-Introduced]] (Ch 1) | drafted | 2,800 | 2026-05-27 |
| [[01-03-Methodology-Note]] (Ch 1) | drafted | 2,000 | 2026-05-28 |
| [[02-01-Antarctic-Opening-Scene]] (Ch 2) | drafted | 1,200 | 2026-05-29 |
| [[02-02-1959-Treaty-Conditions]] (Ch 2) | drafted | 2,500 | 2026-05-30 |
| [[02-03-The-Treaty-Today]] (Ch 2) | drafted | 3,800 | 2026-05-31 |
| [[02-04-Treaty-Analyzed-Against-Conditions]] (Ch 2) | drafting | 3,300 | 2026-05-31 |
| [[03-01-Hoshi-Opening]] (Ch 3) | drafting | 1,750 | 2026-06-02 |
| [[03-02-Family-Business-Statistics]] (Ch 3) | planned | 0 | — |
| [[03-03-Four-Conditions-As-Applied-To-Families]] (Ch 3) | planned | 0 | — |

### Threads (4 active)

| Thread | Status | Sections engaged |
|--------|--------|------------------|
| [[Distributed-Legitimacy]] | active | 5 |
| [[Adaptive-Ritual]] | active | 3 |
| [[Selection-Pressure-Hostile-But-Not-Lethal]] | active | 4 |
| [[Written-Tradition-Allows-Reconstruction]] | active | 3 |

### Characters

*Non-fiction cartridge — Character Items not used. Named individuals (e.g., Hoshi family generations) are referenced inline in Section Items.*

### Readers (3 active — new in v1.1)

| Reader | Status | Priority |
|--------|--------|----------|
| [[Skeptic]] | active | primary |
| [[Impatient-Generalist]] | active | primary |
| [[Domain-Expert]] | active | primary |

READER-SIMULATION sessions: none yet run (the activity is new in v1.1; planned for after Chapter 3 first-draft completes).

### Sources

12 ingested sources. See `Items/Sources/` for full list. Most-cited:

- [[Beard-SPQR-2015]] — for Roman case
- [[Tainter-Collapse-1988]] — counter-image of the argument + methodological grounding
- [[Acemoglu-Robinson-Why-Nations-Fail-2012]] — institutional framework
- [[Hoshi-Ryokan-Records]] — Chapter 3 primary-source corpus

### Notes (recent)

8 unplaced notes; 5 placed. See `Items/Notes/`.

## Open Threads (to address next session)

- [ ] Complete Section 3.1 outline (4 beats currently planned; may need a 5th)
- [ ] Decide whether the Hoshi Ryokan opening scene should be in present tense (immediate) or past tense (historical-distance) — voice consideration
- [ ] Verify Hoshi family founding-year claim (currently 718 AD) against primary sources before drafting prose
- [ ] Chapter 2 is over target word count — schedule structural revision pass after Chapter 3 first-draft complete
- [ ] **NEW (v1.1):** First READER-SIMULATION on Chapter 2 (drafted) using all three Reader Items; planned for after Chapter 3 first-draft completes
- [ ] **NEW (v1.1):** First ARGUMENT-AUDIT on `_argument.md` v2 — particularly pressure-test the independence concerns flagged for Sub-claims 1+4 and the weakest-link assessment for Sub-claim 4
- [ ] **NEW (v1.1):** Confirm proposed practice focus "landing-the-closing-claim" with writer at next CRAFT-REVIEW
- [ ] **NEW (v1.1):** Propose creating `_craft-profile.md` at OV root once a second cartridge exists OR end of Chapter 3 first-draft, whichever comes first

## Stuck Flags

*(none active)*

## Revision Pass Progress

- **Current pass:** none
- **Current round:** 0
- **Planned next pass:** Structural revision of Chapter 2 (over-target word count) after Chapter 3 first-draft

## Recent Sessions

- [[2026-06-02_014_OUTLINE]] — Chapter 3 Section 3.1 beat-level outline
- [[2026-05-31_013_DRAFT]] — Chapter 2 Section 04 partial draft
- [[2026-05-30_012_DRAFT]] — Chapter 2 Section 03 complete
- [[2026-05-29_011_DRAFT]] — Chapter 2 Section 02 complete

## User Preferences (observed)

- Sessions tend to land in the 75–90 min range despite the stated 60–90 target
- Voice-samples mode is being used productively; the AI's offered drafts get revised about 40% before the writer accepts them
- The writer prefers OUTLINE sessions on Mondays (sets up the week's drafting)
- STUCK-DIAGNOSTIC has not been needed yet (project is still in momentum)

## Quality gates for drafting stage

- [x] Outline at chapter level complete
- [x] Outline at section level for current chapter (Chapter 3) complete
- [ ] All Chapter 1 sections at `drafted` status — DONE
- [ ] All Chapter 2 sections at `drafted` status — 3 of 4 done; Section 04 in progress
- [ ] Chapter 3 first-draft — in progress
- [ ] First read-through after first-draft complete
- [ ] First structural revision pass
